...
CREATE TABLE IF NOT EXISTS tipos_pago (
    ...
    mes_pension VARCHAR(20), -- 'marzo', 'abril', etc. (solo para pensiones)
    orden INTEGER NOT NULL,
    activo BOOLEAN DEFAULT true,
    created_at TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS archivos_subidos (
    ...
    tipo_archivo VARCHAR(50) NOT NULL, -- 'excel', 'pdf'
...
CREATE TABLE IF NOT EXISTS estadisticas_cache (
    ...
    periodo VARCHAR(50) NOT NULL, -- '2024', '2024-03', etc.
...
CREATE TRIGGER update_deudas_updated_at BEFORE UPDATE ON deudas FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
LEFT JOIN pagos p ON a.id = p.alumno_id AND p.año_academico = 2024
...
FROM alumnos a
LEFT JOIN grados g ON a.grado_id = g.id
LEFT JOIN deudas d ON a.id = d.alumno_id AND d.año_academico = 2024
...
FROM grados g
LEFT JOIN alumnos a ON g.id = a.grado_id AND a.activo = true AND a.año_academico = 2024
LEFT JOIN pagos p ON a.id = p.alumno_id AND p.año_academico = 2024
LEFT JOIN deudas d ON a.id = d.alumno_id AND d.año_academico = 2024
